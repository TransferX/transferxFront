...
import { BiTransfer, BiSolidDashboard, BiQrScan, BiBell, BiWrench, BiHelpCircle, BiExit      } from "react-icons/bi";
...
const Sidebar = () => {
  ...
  return (
    <>
      <div
        className={`bg-cinzaMedio h-screen p-5 pt-8 ${
          open ? "w-72" : "w-20"
      ...
      >
        ...
        <ul className="pt-2">
            {Menus.map((menu, index) => (
                <>
                <li key={index} className={`text-gray-300 text-sm flex items-center gap-x-4 cursor-pointer p-2 hover:bg-branco/20 rounded-md mt-2 ${menu.spacing ? "mt-9" : "mt-2"}`}>
